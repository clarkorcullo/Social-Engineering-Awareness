...
def update_final_assessment():
    """Update final assessment questions without affecting user data"""
    with app.app_context():
        print("🔄 Starting final assessment update process...")
        
        # Check if final assessment questions exist
        existing_questions = FinalAssessmentQuestion.query.count()
        print(f"📊 Found {existing_questions} existing final assessment questions")
        
        if existing_questions > 0:
            print("🔄 Updating existing questions...")
            update_existing_questions()
        else:
            print("📝 Creating new final assessment questions...")
            create_new_questions()
        
        print("✅ Final assessment update completed successfully!")

def update_existing_questions():
    """Update existing final assessment questions with cleaner formatting"""
    questions_to_update = [
        {
            'id': 1,
    ...
    for question_data in questions_to_update:
        question = FinalAssessmentQuestion.query.filter_by(id=question_data['id']).first()
        if question:
    ...
    db.session.commit()
    print(f"✅ Updated {updated_count} questions successfully")

def create_new_questions():
    """Create new final assessment questions"""
    questions = [
